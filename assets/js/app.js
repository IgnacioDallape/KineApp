...
// Ícono de sobre (mail). Hereda el color del texto del botón (fill:currentColor).
function mailIcon(size) {
  size = size || 15;
  return `<svg viewBox="0 0 24 24" width="${size}" height="${size}" fill="currentColor" style="vertical-align:-3px;flex-shrink:0" aria-hidden="true"><path d="M20 4H4c-1.1 0-1.99.9-1.99 2L2 18c0 1.1.9 2 2 2h16c1.1 0 2-.9 2-2V6c0-1.1-.9-2-2-2zm0 4l-8 5-8-5V6l8 5 8-5v2z"/></svg>`;
}
...
// Envía la rutina como PDF ADJUNTO por WhatsApp: el archivo queda dentro del chat,
// sin links ni descargas — el paciente no tiene que ir a ningún lado.
async function _enviarRutinaPDF(p, bloques, scopeLabel) {
  ...
  if (!out || !out.blob) { alert('No se pudo generar el PDF de la rutina. Revisá tu conexión e intentá de nuevo.'); return; }

  // 1) Preferido (celular): adjuntar el PDF de verdad. En el menú de compartir elegís
  //    WhatsApp y el archivo queda en el chat — sin links, sin abrir nada.
  if (navigator.canShare) {
    try {
      const file = new File([out.blob], out.filename, { type: 'application/pdf' });
      if (navigator.canShare({ files: [file] })) {
        await navigator.share({ files: [file], title: out.filename, text: mensaje });
        return;
      }
    } catch (err) { if (err && err.name === 'AbortError') return; }   // cerró el menú de compartir
  }

  // 2) Fallback (compu sin compartir archivos): descargamos el PDF y abrimos WhatsApp con el mensaje.
  ...
  abrirWhatsAppUrl(`https://wa.me/${tel}?text=${encodeURIComponent(mensaje + '\n\n(Adjuntamos la rutina en PDF: sumá el archivo que acabamos de descargar.)')}`);
}
  ...
  document.getElementById('ficha-content').innerHTML = `
    ...
    <div class="ficha-actions">
      <div class="fa-group">
        ...
        <button class="btn btn-secondary" onclick="abrirInformePaciente('${p.id}')">📄 Ver informe</button>
      </div>
      <div class="fa-group fa-share">
        <span class="fa-send-label">Compartir:</span>
        <button class="btn btn-success" onclick="compartirInformeWhatsApp('${p.id}')">${waIcon()} WhatsApp</button>
        <button class="btn btn-mail" onclick="compartirInformeMail('${p.id}')">${mailIcon()} Mail</button>
      </div>
    </div>
